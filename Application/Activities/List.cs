                ...
                CancellationToken cancellationToken)
            {
                ...
                if (request.IsGoing && !request.IsHost)
                    queryable = queryable.Where (x =>
                        x.UserActivities.Any (a => a.AppUser.UserName == _userAccesor.GetCurrentUsername ()));

                if (request.IsHost && !request.IsGoing)
                    queryable = queryable.Where (x =>
                        x.UserActivities.Any (a => a.AppUser.UserName == _userAccesor.GetCurrentUsername () && a.IsHost));
